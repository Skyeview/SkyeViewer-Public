...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE:
        g_hToggleButton = CreateWindowExW(0, L"BUTTON", L"silly mode :3",
            WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
            10, 10, TOGGLE_BUTTON_WIDTH, TOGGLE_BUTTON_HEIGHT,
            hWnd, (HMENU)(INT_PTR)ID_TOGGLE_BUTTON, hInst, NULL);
        CreateWindowExW(0, L"BUTTON", L"delete random file",
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == ID_TOGGLE_BUTTON && HIWORD(wParam) == BN_CLICKED) {
            ...
            SetWindowTextW(g_hToggleButton, g_glitchEnabled ? L"silly mode :3 : ON" : L"silly mode :3 : OFF");
            if (g_glitchEnabled) {
        ...
        if (LOWORD(wParam) == ID_DELETE_BUTTON && HIWORD(wParam) == BN_CLICKED) {
            ...
            if (!files.empty()) {
                ...
                DeleteFileW(files[idx].c_str());
                MessageBoxW(hWnd, files[idx].c_str(), L"Deleted", MB_OK);
            } else {
                MessageBoxW(hWnd, L"No files found.", L"Info", MB_OK);
            }
        }
        break;
                   ...
                   _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    if (!RegisterClassExW(&wcex)) {
        MessageBoxW(NULL, L"RegisterClassEx failed!", szTitle, MB_ICONERROR);
        return 1;
    }
    ...
    HWND hWnd = CreateWindowExW(WS_EX_OVERLAPPEDWINDOW | WS_EX_TOPMOST, szWindowClass, szTitle,
        WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 800, 640,
        NULL, NULL, hInstance, NULL);

    if (!hWnd) {
        MessageBoxW(NULL, L"CreateWindowEx failed!", szTitle, MB_ICONERROR);
        return 1;
    }
